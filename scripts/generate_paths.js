...
const bgImages = fg
  .globSync(ROOT + '/bg/**/bg-*.jpg', { dot: true })
  .map((p) => p.split(ROOT)[1]);
const vrmList = fg
  .globSync(ROOT + '/vrm/**/*.vrm', { dot: true })
  .map((p) => p.split(ROOT)[1]);
const speechT5SpeakerEmbeddingsList = fg
  .globSync(ROOT + '/speecht5_speaker_embeddings/**/*.bin', { dot: true })
  .map((p) => p.split(ROOT)[1]);

...
export async function localXTTSTTS(message: string) {
  ...
  console.log('[AllTalk] Config values:', {
    ...
    speed: config("alltalk_speed"),
  });

  const formData = new URLSearchParams({
    text_input: message,
    text_filtering: 'standard',
    character_voice_gen: config("alltalk_voice") || 'friendly.wav',
    narrator_enabled: 'false',
    narrator_voice_gen: config("alltalk_voice") || 'friendly.wav',
    text_not_inside: 'character',
    language: config("alltalk_language") || 'en',
    speed: config("alltalk_speed") || '1.2',

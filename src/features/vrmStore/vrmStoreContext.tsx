...
export const VrmStoreProvider = ({ children }: PropsWithChildren<{}>): JSX.Element => {
  ...
  const vrmListAddFile = (file: File, viewer: Viewer) => {
    vrmListDispatch({
      type: VrmStoreActionType.addItem,
      itemFile: file,
      callback: (callbackProp: AddItemCallbackType) => {
        viewer
          .loadVrm(callbackProp.url)
          .then(() => {
          ...
          .then(() => {
            ...
            viewer.getScreenshotBlob((thumbBlob: Blob | null) => {
              if (!thumbBlob) return;
              vrmListDispatch({
                type: VrmStoreActionType.updateVrmThumb,
                url: callbackProp.url,
                thumbBlob,
                vrmList: callbackProp.vrmList,
                callback: (updatedThumbVrmList: VrmData[]) => {
                  vrmListDispatch({
                    type: VrmStoreActionType.setVrmList,
                    vrmList: updatedThumbVrmList,
                  });
                },
              });
            });
  ...
  useEffect(() => {
    vrmListDispatch({
      type: VrmStoreActionType.loadFromLocalStorage,
      vrmList: vrmInitList,
      callback: (updatedVmList: VrmData[]) => {
  ...
  const getCurrentVrm = () => {
    return config('vrm_save_type') == 'local'
      ? loadedVrmList.find((vrm) => vrm.getHash() == config('vrm_hash'))
      : loadedVrmList.find((vrm) => vrm.url == config('vrm_url'));
  };

  return (
    <VrmStoreContext.Provider
      value={{
        getCurrentVrm: getCurrentVrm,

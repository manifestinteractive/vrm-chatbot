...
export const vrmStoreReducer = (
  state: VrmData[],
  action: VrmDispatchAction,
): VrmData[] => {
  let newState = state;
  switch (action.type) {
    ...
    case VrmStoreActionType.setVrmList:
      if (action.vrmList && action.vrmList.length) newState = action.vrmList;
    case VrmStoreActionType.loadFromLocalStorage:
      if (action.vrmList && action.callback) newState = LoadFromLocalStorage(action);
    default:
      break;
  }
  return newState;
...
const addItem = (
  ...
  BlobToBase64(blob).then((data: string) => {
    ...
    if (loadedVrmList.findIndex((vrm: VrmData) => vrm.hashEquals(hash)) == -1) {
      loadedVrmList = [
        ...loadedVrmList,
        new VrmData(hash, url, '/vrm/thumb-placeholder.jpg', 'local'),
      ];
...
const updateVrmThumb = (vrmList: VrmData[], action: VrmDispatchAction): VrmData[] => {
  if (!(action.url && action.thumbBlob && action.vrmList)) return vrmList;
  let newVrmList = new Array<VrmData>(...action.vrmList);
  const vrm = newVrmList.find((vrm: VrmData) => {
    return vrm.url == action.url;
  });
...
const LoadFromLocalStorage = (action: VrmDispatchAction): VrmData[] => {
  vrmDataProvider
    ...
    .then((vrmDataArray: VrmData[]): VrmData[] => {
      ...
      newList = newList.filter((value, index, array) => {
        return (
          index ==
          array.findIndex((vrm) => {
            return vrm.getHash() == value.getHash();
          })
        );
      });
      return newList;
    ...
    .then((data: VrmData[]) => {
      if (action.callback && data) action.callback(data);
    });

  return action.vrmList || new Array<VrmData>();
};

const VrmDataArrayFromVrmDbModelArray = async (
  vrms: VrmDbModel[],
): Promise<VrmData[]> => {
  const promiseArray = vrms.map((vrmDbModel: VrmDbModel): Promise<VrmData> => {
    return VrmDbModelToVrmData(vrmDbModel);
  });
  return Promise.all(promiseArray).then((vrmDataArray: VrmData[]) => {
    return vrmDataArray;
  });
};

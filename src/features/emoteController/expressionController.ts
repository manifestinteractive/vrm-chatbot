...
export class ExpressionController {
  ...
  public registerExpression(vrm: VRM) {
    ...
    const { expressionMap: allExpressions, presetExpressionMap: presetExpressions } =
      expressionManager;
    const oddExpressions = Object.keys(allExpressions).filter(
      (key) => !(key in presetExpressions),
    );
  ...
  public playEmotion(preset: VRMExpressionPresetName | string) {
    ...
    const value =
      normalizedPreset in VRMExpressionPresetName
        ? normalizedPreset === 'Surprised'
          ? 0.5
          : 1
        : 0.5;

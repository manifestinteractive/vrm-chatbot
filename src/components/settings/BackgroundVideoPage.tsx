...
export function BackgroundVideoPage({
  ...
  const description = (
    <>
      {t(
        'bg_youtube_desc',
        `Select a background video. Copy this from the YouTube video ID from the URL, it will look something like kDCXBwzSI`,
      )}
    </>
  );

  return (
    <BasicPage title={t('Background Video')} description={description}>
          ...
          <FormRow label={t('YouTube Video ID')}>
            ...
            {videoChanged && (
              <p className="text-xs text-gray-500 mt-4">
                Video changed.{' '}
                <span
                  className="text-cyan-500 hover:text-cyan-600 hover:cursor-pointer"

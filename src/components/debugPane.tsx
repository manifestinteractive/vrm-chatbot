        ...
        <div className="p-2 bg-white">
          ...
          <div className="inline-block ml-2 items-center">
            <span className="px-1">
              ...
              <span className="text-xs text-gray-400">{config('chatbot_backend')}</span>
            </span>
            <span className="px-1">
              <span className="text-xs text-gray-500">tts: </span>
              <span className="text-xs text-gray-400">{config('tts_backend')}</span>
            </span>
            <span className="px-1">
              <span className="text-xs text-gray-500">stt: </span>
              <span className="text-xs text-gray-400">{config('stt_backend')}</span>
            </span>
            <span className="px-1">
        ...
        <div className="relative w-full max-h-screen overflow-y-scroll inline-block px-2 md:px-8">
          {(window as any).error_handler_logs
            ...
            .map((log: any, idx: number) => (
              ...
              >
                {log.type === 'debug' && (
                  <span className="inline-flex items-center rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 font-mono w-12">
                    debug
                  </span>
                )}
                {(log.type === 'info' || log.type === 'log') && (
                  <span className="inline-flex items-center rounded-md bg-green-100 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/20 font-mono w-12">
                    info
                  </span>
                )}
                {log.type === 'warn' && (
                  <span className="inline-flex items-center rounded-md bg-yellow-50 px-2 py-1 text-xs font-medium text-yellow-800 ring-1 ring-inset ring-yellow-600/20 font-mono w-12">
                    warn
                  </span>
                )}
                {log.type === 'error' && (
                  <span className="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10 font-mono w-12">
                    error
                  </span>
                )}

                <small className="text-gray-400 px-1 font-mono">
                  {(log.ts / 1000) | 0}
                </small>
